...
def generate_report_text(data):
    ...
    def corr_desc(r):
        ...
        return "相关性弱"
    L = []
    L.append("# " + school + " 英语AI听说产品应用成效报告\n")
    L.append("**生成时间：" + datetime.now().strftime("%Y年%m月%d日") + "**\n")
    L.append("---\n")
    L.append("## 一、封面信息\n")
    L.append("| 项目 | 内容 |\n|------|------|\n")
    L.append("| 学校名称 | " + school + " |\n")
    L.append("| 所属省份 | " + data.get("province", "黑龙江省") + " |\n")
    L.append("| 所属城市 | " + data.get("city", "哈尔滨市") + " |\n")
    L.append("| 参与班级数 | " + str(data.get("classes", 0)) + "个 |\n")
    L.append("| 激活学生总数 | " + str(data.get("total_students", 0)) + "人 |\n")
    L.append("| 数据周期 | " + mr + " |\n")
    L.append("\n")
    L.append("## 二，前言与背景\n")
    L.append("**课程标准：** 《义务教育课程方案和课程标准（2022年版）》明确提出，要培养学生核心素养，强调语言运用能力，尤其是听说能力的培养。英语听说教学是落实学科核心素养、提升学生综合语言运用能力的重要途径，日常朗读、跟读训练和模拟测试是提升学生听说能力的有效手段。\n\n")
    L.append("**地方政策：** 黑龙江省积极响应国家教育改革，从2026年起将实行全省统一中考命题，外语科目纳入口语、听力测试并计入中考总分。哈尔滨市于2024年发布中考综合改革实施方案，明确英语听说考试于人机对话形式进行，过渡期为2024-2025年，2026年起全面实施。中考英语听说占比不低于20分，倒逼初中阶段加强日常听说训练。\n\n")
    L.append("---\n")
    L.append("## 三、激活/应用概况\n")
    L.append("本阶段，" + school + "共计" + str(data.get("classes", 0)) + "个班级、" + str(data.get("total_students", 0)) + "名学生全面激活并投入使用，注册学生覆盖率达100%。\n\n")
    L.append("| 指标 | 数值 |\n|------|------|\n")
    L.append("| 参与学校数 | " + str(data.get("schools", 0)) + "所 |\n")
    L.append("| 班级数（去重） | " + str(data.get("classes", 0)) + "个 |\n")
    L.append("| 激活学生总数 | " + str(data.get("total_students", 0)) + "人 |\n")
    L.append("| 布置作业次数（合计） | " + str(data.get("assign_count", 0)) + "次 |\n")
    L.append("| 布置作业份数（合计） | " + str(data.get("assign_total", 0)) + "份 |\n")
    L.append("\n> 数据来源：班级数据总览、作业明细\n\n")
    L.append("---\n")
    L.append("## 四、应用情况分析\n")
    ...
    for cat, cnt in sorted(data.get("category_counts", {}).items(), key=lambda x: -x[1]):
        ...
        L.append("| **" + cat + "** | " + str(cnt) + "次 | " + str(pct_v) + "% | " + cat_meta.get(cat, "") + " |\n")
    L.append("\n**子类分布：**\n")
    ...
    for cat in ["同步", "专项", "模拟", "课外拓展"]:
        ...
        txt = "、".join([str(k[1] if isinstance(k, tuple) else k) + "(" + str(v) + "次)" for k, v in subs])
        if txt:
            L.append("- **" + cat + "类**：" + txt + "\n")
    L.append("\n")
    L.append("### 4.2 整体应用数据\n")
    L.append("从整体使用数据来看，学生主动练习意愿强烈，词汇自主练习次数高达**" + str(vocab_p) + "次**，说明产品有效激发了学生自主学习行为。教师布置作业覆盖面广，班级作业布置次数合计达" + str(data.get("assign_count", 0)) + "次。\n\n")
    L.append("| 指标 | 数值 |\n|------|------|\n")
    L.append("| 布置作业次数 | " + str(data.get("assign_count", 0)) + "次 |\n")
    L.append("| 布置作业份数 | " + str(data.get("assign_total", 0)) + "份 |\n")
    L.append("| 作业完成率（均值） | " + str(data.get("completion_rate", 0)) + "% |\n")
    L.append("| 班级平均作业得分率 | " + str(data.get("score_rate_avg", 0)) + "% |\n")
    L.append("| 学生自主练习次数 | " + str(data.get("self_practice", 0)) + "次 |\n")
    L.append("| 词汇自主练习次数 | " + str(vocab_p) + "次 |\n")
    L.append("\n")
    L.append("### 4.3 应用频次分析\n")
    L.append("**整体趋势：** 作业使用呈现「脉冲式」节奏——" + str(total_hw) + "次作业分布在" + str(len(months)) + "个月份，2026年1月使用量激增至峰值，与期末复习节奏同步，说明产品使用与学校教学周期高度吻合。\n\n")
    L.append("**各年级月度作业量：** 各年级使用节奏一致，均以1月为最高峰，师生在期末复习阶段对产品依赖度最高，这是积极信号。\n\n")
    L.append("**月作业总量（折线图见图表页）：**\n\n")
    L.append("| 月份 | 作业数 | 趋势 |\n|------|--------|------|\n")
    for i, m in enumerate(months):
        cnt = data.get("monthly_hw", {}).get(m, 0)
        prev = data.get("monthly_hw", {}).get(months[i-1], 0) if i > 0 else 0
        trend = "—" if i == 0 else ("↑" if cnt > prev else "↓")
        L.append("| " + m + " | " + str(cnt) + " | " + trend + " |\n")
    L.append("\n**各年级月度作业量分布：**\n\n")
    grade_hw = data.get("grade_monthly_hw", {})
    L.append("| 月份 | 六年级 | 七年级 | 八年级 |\n|------|--------|--------|--------|\n")
    for m in months:
        g6 = str(grade_hw.get("六年级", {}).get(m, 0))
        g7 = str(grade_hw.get("七年级", {}).get(m, 0))
        g8 = str(grade_hw.get("八年级", {}).get(m, 0))
        L.append("| " + m + " | " + g6 + " | " + g7 + " | " + g8 + " |\n")
    L.append("\n")
    L.append("### 4.4 应用方式分析\n")
    L.append("从作业内容结构来看，**同步训练**（课文朗读/跟读）是学生日常接触最多的形式，合计占比高达**" + str(syn_pct) + "%**，构成学生每日开口说英语的基础；**专项训练**（听说专项）占比**" + str(sub_pct) + "%**，用于考前针对性强化；**模拟训练**（听说模拟题）占比**" + str(mon_pct) + "%**，直接服务听说考试备考。这种「日常打基础 + 考前专项强化 + 模拟实战」的组合模式，是科学备考的正确路径。\n\n")
    L.append("| 大类 | 占比 | 核心子类及次数 |\n|------|------|----------------|\n")
    for cat in ["同步", "专项", "模拟", "课外拓展"]:
        pct_v = data.get("category_pct", {}).get(cat, 0)
        subs = sorted([(k, v) for k, v in sub.items() if isinstance(k, tuple) and k[0] == cat], key=lambda x: -x[1])[:3]
        top3 = "、".join([str(k[1] if isinstance(k, tuple) else k) + "(" + str(v) + "次)" for k, v in subs])
        L.append("| " + cat + " | " + str(pct_v) + "% | " + top3 + " |\n")
    L.append("\n")
    L.append("---\n")
    L.append("## 五、应用效果分析\n")
    ...
    for m, score in sorted(data.get("mock_hw_score_monthly", {}).items()):
        L.append("| " + m + " | " + str(score) + "% |\n")
    L.append("\n**分年级得分率趋势：**\n\n")
    for grade, monthly in sorted(grade_scores.items()):
        vals = [m + ":" + str(s) + "%" for m, s in sorted(monthly.items())]
        L.append("- **" + grade + "**：" + " → ".join(vals) + "\n")
    L.append("\n")
    L.append("### 5.2 相关性分析\n")
    ...
    for lbl, r, n in [
        ...
        L.append("| " + lbl + " | " + "%.4f" % r + " | " + str(n) + "个班级 | " + d + flag + " | " + ("正向关联" if r > 0.3 else "需进一步观察") + " |\n")
    L.append("\n")
    strong = data.get("strong_corrs", [])
    if strong:
        L.append("**强相关发现：**\n")
        for lbl, r, n in strong:
            L.append("- **" + lbl + "**与得分率呈中强正相关（r=" + "%.4f" % r + "，n=" + str(n) + "），" + school + "在自主学习行为建设上已初步形成正向循环——越主动练习的学生，得分表现越优异。\n")
        L.append("\n")
    else:
        best = max([("词汇自主练习", r_v), ("作业完成率", r_c), ("自主练习", r_s)], key=lambda x: abs(x[1]))
        L.append("**分析：** " + best[0] + "与得分率相关性最强（r=" + "%.4f" % best[1] + "），正向关联趋势明显，自主学习行为对成绩提升具有积极意义。\n\n")
    L.append("## 六、典型案例/学校分析\n")
    if top:
        L.append("以**" + tc_name + "（" + str(data.get("top_class_grade", "")) + "）**作为标杆班级：该班在数据周期内共完成**" + str(top[0]["hw_count"]) + "次**听说模拟作业，平均得分率高达**" + "%.2f" % top_score + "%**，居全校前列。\n\n")
        L.append("**标杆班级月度得分率走势（折线图见图表页）：**\n\n")
        L.append("| 月份 | 得分率 | 作业量 |\n|------|--------|--------|\n")
        top_m = data.get("top_class_monthly", {})
        for m in sorted(top_m.keys()):
            v = top_m[m]
            L.append("| " + m + " | " + str(v["score"]) + "% | " + str(v["count"]) + "次 |\n")
        L.append("\n**全校TOP5班级（听说模拟作业次数）：**\n\n")
        L.append("| 排名 | 班级 | 年级 | 作业次数 | 平均得分率 |\n|------|------|------|--------|--------|\n")
        for i, c in enumerate(top, 1):
            L.append("| " + str(i) + " | " + c["class_name"] + " | " + c["grade"] + " | " + str(c["hw_count"]) + "次 | " + "%.2f" % c["avg_score"] + "% |\n")
        L.append("\n")
    L.append("---
")
    L.append("## 七、总结与建议\n")
    L.append("### 7.1 主要亮点\n\n")
    L.append("**亮点一：激活率高、使用面广。** 全校" + str(data.get("total_students", 0)) + "名学生、" + str(data.get("classes", 0)) + "个班级全面激活，注册使用率达100%，学校整体应用基础扎实。\n\n")
    L.append("**亮点二：词汇自主练习异常活跃。** 全校词汇自主练习累计达" + str(vocab_p) + "次，生均超过18次，充分说明产品有效激发了学生的自主学习意愿，词汇自主练习与得分率相关系数r=" + "%.4f" % r_v + "，说明自主练习越多的学生得分越高——这一正向循环是教学最希望看到的局面。\n\n")
    L.append("**亮点三：同步训练体系完善。** 课文朗读、跟读等日常同步训练占总作业量的" + str(syn_pct) + "%，帮助学生在日常学习中夯实语音基础，形成持续性开口习惯。\n\n")
    L.append("**亮点四：模拟实战训练稳定。** 听说模拟（含区域精选、单元测试、中考冲刺等）占总作业量" + str(mon_pct) + "%，考前冲刺训练体系化，直接服务中考备考。\n\n")
    if top:
        L.append("**亮点五：标杆班级示范作用突出。** " + tc_name + "平均得分率高达" + "%.2f" % top_score + "%，展示了高频训练与高分之间的正向关系，为全校提供可复制的经验。\n\n")
    L.append("### 7.2 建议\n\n")
    L.append("| 优先级 | 维度 | 建议内容 |\n|--------|------|----------|\n")
    L.append("| 🟡 中 | 提升完成率 | 当前作业完成率均值仍有空间，可通过分层任务设计，确保各层次学生均能完成 |\n")
    L.append("| 🟡 中 | 保持自主练习优势 | 词汇自主练习是亮点，建议持续激励，如设立「自主练习之星」等正向反馈机制 |\n")
    L.append("| 🟡 中 | 深化专项训练 | 专项听说练习占比" + str(sub_pct) + "%，可适当增加针对性薄弱题型的专项突破训练 |\n")
    L.append("| 🟢 低 | 推广标杆经验 | 总结" + tc_name + "的练习模式，形成可复制的优秀班级经验进行全校推广 |\n")
    L.append("\n---\n")
    L.append("*数据周期：" + mr + " | 生成时间：" + datetime.now().strftime("%Y年%m月%d日 %H:%M") + "\n")

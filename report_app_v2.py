...
def make_charts(data: dict) -> dict:
    ...
    # 图1: 月度作业总量折线图
    totals = [data.get('monthly_hw', {}).get(m, 0) for m in months]
    fig = go.Figure()
    ...
    if mock_scores:
        ...
        fig5.add_trace(go.Scatter(
            ...
            text=['%.1f%%' % s for s in sc], textposition='top center', textfont=dict(size=11),
            name='得分率'
        ))
        fig5.update_layout(
    ...
    if top_m:
        ...
        fig7.add_trace(go.Scatter(
            ...
            text=['%.1f%%' % s for s in sc_t], textposition='top center', textfont=dict(size=10),
            yaxis='y'
        ))
        ...
        fig7.update_layout(
            title=dict(text='图7 ' + data.get('top_class_name', '') + '月度得分率与作业量', font=dict(size=16)),
            template='plotly_white', height=380,
            legend=dict(orientation='h', yanchor='bottom', y=1.02),
            hovermode='x unified'
        )
        fig7.update_y2(title_text='作业次数', overlaying='y', side='right')
...
if 'report' in st.session_state:
    ...
    with tab2:
        ...
        st.divider()
        chart_map = [
            ('monthly_line', '图1 月度作业总量趋势（折线图）'),
            ('grade_monthly_line', '图2 各年级月度作业量趋势'),
            ('cat_stacked', '图3 月度大类分布堆叠图'),
            ('cat_pie', '图4 作业类型占比'),
            ('mock_score', '图5 听说模拟得分率趋势'),
            ('grade_score', '图6 各年级听说模拟得分率对比'),

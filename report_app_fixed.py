...
def analyze_data(class_df, hw_df):
    ...
    results['score_rate_avg'] = round(float(class_df['作业得分率'].mean()) * 100, 2)
...
PROVINCE_POLICY = (
    "黑龙江省积极响应国家教育改革，从2026年起将实行全省统一中考命题，外语科目纳入口语、听力测试并计入中考总分。"
    "哈尔滨市于2024年发布中考综合改革实施方案，明确英语听说考试于人机对话形式进行，过渡期为2024-2025年，2026年起全面实施。"
    "中考英语听说占比不低于20分，倒逼初中阶段加强日常听说训练。"
)
...
def export_to_docx(report_md):
    ...
    def rfont(run, fname, fsize, bold=False):
        run.font.name = fname
    ...
    def add_head(text, fname='黑体', fsize=16, bold=True, align=WD_ALIGN_PARAGRAPH.LEFT):
